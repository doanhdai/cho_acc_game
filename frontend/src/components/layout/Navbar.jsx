    ...
    <nav className="navbar">
      <div className="nav-inner container">
        ...
        <Link to="/" className="nav-logo">
          <img src={import.meta.env.VITE_LOGO_URL || "/logo_lienquan.png"} alt="Logo" style={{ width: 40, height: 40, objectFit: 'contain' }} />
          <span className="logo-text">{import.meta.env.VITE_SITE_NAME || "Shop Acc Game"}</span>
        </Link>
      ...
      {menuOpen && (
        <div className="mobile-menu">
          ...
          {user ? (
            <>
              <Link to="/profile" className="mobile-link" onClick={() => setMenuOpen(false)} style={{ display: 'flex', alignItems: 'center', gap: 10 }}><FiUser /> Thông Tin Cá Nhân</Link>
              <Link to="/deposit" className="mobile-link" onClick={() => setMenuOpen(false)} style={{ display: 'flex', alignItems: 'center', gap: 10 }}><GiDiamondHard /> Nạp Tiền</Link>
              <Link to="/history" className="mobile-link" onClick={() => setMenuOpen(false)} style={{ display: 'flex', alignItems: 'center', gap: 10 }}><FiClock /> Lịch Sử</Link>
              <Link to="/my-accounts" className="mobile-link" onClick={() => setMenuOpen(false)} style={{ display: 'flex', alignItems: 'center', gap: 10 }}><FiShoppingBag /> Acc Của Tôi</Link>
              {user.role === 'admin' && <Link to="/admin" className="mobile-link" onClick={() => setMenuOpen(false)} style={{ display: 'flex', alignItems: 'center', gap: 10 }}><FiSettings /> Admin</Link>}
