...
import { userAPI, default as api } from '../../api';
    ...
    <div style={{ padding: '40px 0', background: 'var(--bg-dark)', minHeight: 'calc(100vh - 64px)' }}>
      <div className="container" style={{ maxWidth: 1140 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 28, flexWrap: 'wrap', gap: 16 }}>
          ...
          <Link to="/sell" className="btn btn-primary" style={{ gap: 8, borderRadius: 'var(--radius)', padding: '10px 18px', fontWeight: 600 }}>
            <FiPlus /> Đăng Tin Mới
          </Link>
        </div>
        ...
        {filteredListings.length === 0 ? (
          <div className="card" style={{ textAlign: 'center', padding: '64px 20px', color: 'var(--text-muted)', borderRadius: 'var(--radius-lg)' }}>
            ...
            <Link to="/sell" className="btn btn-primary" style={{ display: 'inline-flex', gap: 8, margin: '0 auto', borderRadius: 'var(--radius)' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
                    ...
                    <div style={{ 
                      ...
                      <div style={{ textAlign: 'right' }}>
                        ...
                        <div style={{ fontSize: 18, fontWeight: 800, color: 'var(--primary)' }}>{formatMoney(acc.price)}</div>
                      </div>
                      ...
                      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                        ...
                        {(acc.status === 'SHOWING' || acc.status === 'PENDING_APPROVAL' || acc.status === 'REJECTED' || acc.status === 'HIDDEN') && (
                          <button
                            onClick={() => navigate(`/sell?edit=${acc.id}`)}
                            className="btn btn-outline btn-sm"
