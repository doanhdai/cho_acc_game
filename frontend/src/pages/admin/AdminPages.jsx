...
import { adminAPI, orderAPI, accountAPI } from '../../api';
...
export function AdminAccounts() {
  ...
  useEffect(() => {
    import('../../api').then(m => {
      m.accountAPI.getCategories().then(r => setCategories(r.data.data));
    }); 
  }, []);
        ...
        <>
          <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', overflow: 'hidden' }}>
            <div className="table-wrap">
              <table>
                <thead><tr><th>ID</th><th>Tên Acc</th><th>Game</th><th>Người bán</th><th>Giá</th><th>Trạng Thái</th><th>Hành Động</th></tr></thead>
                <tbody>
                  {accounts.map(acc => (
                    <tr key={acc.id}>
                ...
                <tbody>
                  {orders.map(o => (
                    <tr key={o.id}>
                      ...
                      <td>
                        {o.status === 'PENDING' && (
                          <button className="btn btn-sm btn-primary" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '6px 8px' }} onClick={() => navigate(`/admin/orders/${o.id}`)} title="Vào Phòng Chat">
                            <FiMessageSquare size={14} />
                          </button>
                        )}
                        {o.status !== 'PENDING' && (
                          <button className="btn btn-sm btn-outline" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '6px 8px' }} onClick={() => navigate(`/admin/orders/${o.id}`)} title="Xem lại">
                            <FiEye size={14} />
                          </button>
                        )}
                      </td>
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
        <button className="btn btn-outline btn-sm" onClick={() => navigate('/admin/orders')} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ← Quay Lại
        </button>
