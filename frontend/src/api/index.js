...
api.interceptors.response.use(
  (res) => res,
  (err) => {
    if (err.response?.status === 401 && !err.config.url.includes('/auth/login') && !err.config.url.includes('/accounts/')) {
...
export const walletAPI = {
  triggerCallback: (data) => api.post('/wallet/callback', data),
};
